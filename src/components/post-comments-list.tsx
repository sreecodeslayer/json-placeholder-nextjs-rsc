import { PostCommentType, PostType, getPostComments } from "@/lib/jsonplaceholder";
import { LoadingSkeleton } from "./skeleton";

export const PostCommentsList = async ({post: {id}}: {post: PostType}) => {
  const comments: PostCommentType[] = await getPostComments(id)

  return (
    <div className="w-full flex flex-col gap-4 overflow-y-auto">
      {comments.map(comment => (
        <div key={comment.id} className="w-full flex flex-col gap-2 p-4 bg-slate-50 rounded-md hover:shadow-lg hover:shadow-slate-200">
...
export const LoadingPostCommentsListSkeleton = () => {
  return <div className="w-full flex flex-col gap-4 overflow-y-auto">
    {[...Array(3)].map((_, index) => (
      <div key={index} className="w-full flex flex-col p-4 bg-slate-50 rounded-md">
